...
$Path = "C:\Transfer\Files"
$Filter = "$Path\*_JDE_RadiaInfoReports.zip"

if ( (Get-ChildItem $Filter).Count -eq 0  )
{
    break
}

if ( Test-Path -Path "$Path\JDE*.txt" )
{
    Remove-Item -Path "$Path\JDE*.txt"
}
if ( Test-Path -Path "\\DEMBMCAPS032SQ2.corp.demb.com\ITAM$\Usr\Interface\Import\ACData\JDE*.txt" )
{
    Remove-Item -Path "\\DEMBMCAPS032SQ2.corp.demb.com\ITAM$\Usr\Interface\Import\ACData\JDE*.txt"
}

$File = (Get-ChildItem -Path $Filter | Sort-Object -Descending).FullName | Select-Object -First 1
Expand-Archive -Path $File -DestinationPath $Path -Force
Remove-Item -Path $Filter

Move-Item -Path "$Path\JDE*.txt" -Destination "\\DEMBMCAPS032SQ2.corp.demb.com\ITAM$\Usr\Interface\Import\ACData"
