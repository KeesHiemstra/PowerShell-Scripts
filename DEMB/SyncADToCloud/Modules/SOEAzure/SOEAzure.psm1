...
<#
    Remove all tags in the RemoveTags array from the input.
#>
function RemoveTags ([string]$Tags, [array]$RemoveTags)
{
...
{
    RemoveTags -Tags $Tags -RemoveTags ($InEnterpriseTags+$InExtraTags)
    ...
    Process
    {
        ...
        if ($HasEnterprisePack)
        {
            ...
            switch -Regex ($Options)
            {
                ...
                "SHAREPOINTENTERPRISE"  { $Result.GranularArray += "[SHA]" }
                "SWAY"                  { $Result.GranularArray += "[SWY]" }
                "SHAREPOINTWAC"         { $Result.GranularArray += "[WAC]" }
...
.EXAMPLE
   Get-AzureLicense Arthur.PendragonPendragon@camelot.ay -ReturnResult Object
...
function Test-AzureLicenseTags
{
    ...
    Process
    {
        ...
        else
        {
            ...
            $EA2 = RemoveTags -Tags $EA2 -RemoveTags (SplitTags -Tags $EA11)
...
function Sort-AzureLicenseTags
{
    ...
    Process
    {
        if ( -not [string]::IsNullOrWhiteSpace($Tags) )
        {
            ...
            $Tags = (SplitTags -Tags $Tags | Sort-Object ) -join ''
